...
public class LogContent {
    ...
    /**
     * See {@link com.nmgtime.logtrace.annos.MethodLog#isOpen()}.
     */
    ...
    LogContent(String methodName, String traceLevel, String mIsOpen, Map<String, JCTree.JCExpression> argMap) {
        this.head = String.format("LOG_TRACE >>>>>> OUTPUT: [TraceId: %s] [method: %s] ",
                MDC.get("trace_id") != null ? MDC.get("trace_id") : "", methodName);
    ...
    public LinkedList<JCTree.JCExpression> getLogParams(Tree.Kind kind,
        ...
        sb.append(this.head).append("[").append(kind.toString().toLowerCase()).append("]").append(" [Line: ")
    ...
    private void processParams(StringBuilder paramContent, LinkedList<JCTree.JCExpression> params,
        ...
        paramMap.forEach((k, v) -> {
            ...
            if (i.incrementAndGet() == paramMap.size()) {
                paramContent.append("{}");
            } else {
                paramContent.append("{}, ");
            }
